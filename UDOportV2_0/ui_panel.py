...
class UDO_PT_ExporterPanel(Panel):
    ...
    def draw(self, context):
        ...
        # FBX Export Section
        layout.separator()
        ui_props = context.scene.udo_ui_props
        
        fbx_box = layout.box()
        fbx_header = fbx_box.row()
        fbx_header.prop(ui_props, "fbx_section_expanded", 
                       icon="TRIA_DOWN" if ui_props.fbx_section_expanded else "TRIA_RIGHT", 
                       icon_only=True, emboss=False)
        fbx_header.label(text="FBX Export", icon='EXPORT')
        
        if ui_props.fbx_section_expanded:
            # Export button
        ...
        anim_header.prop(ui_props, "animation_section_expanded", 
                        icon="TRIA_DOWN" if ui_props.animation_section_expanded else "TRIA_RIGHT", 
                        icon_only=True, emboss=False)
        anim_header.label(text="Blendshape Export", icon='ANIM')
        
        if ui_props.animation_section_expanded:
